...
const MainSection = () => {
  ...
  const storedTeamData = localStorage.getItem('team-array');
   let initialTeamArr = storedTeamData ? JSON.parse(storedTeamData) : [];

   const storedEmpData = localStorage.getItem('emp-array');
   let initialEmpArr = storedEmpData ? JSON.parse(storedEmpData) : empArr;
  let [teamArr, setTeamArr] = useState( initialTeamArr);
  let [empArrrr, setEmpArrrr] = useState(initialEmpArr);
  ...
  useEffect(() => {
    ...
    if (teamArr.length === 0) {
      setAge(sum);
        ...
        let avg=sum/teamArr.length;
        setAge(avg);
    }

    localStorage.setItem('team-array',JSON.stringify(teamArr))
    localStorage.setItem('emp-array',JSON.stringify(empArrrr))
  }, [age,teamArr,empArrrr]);

  function addEmployee(item, uuid) {
    setTeamArr([...teamArr, item]);
    let filterEmpArr = empArrrr.filter((item) => {
      return uuid != item.id;
    });
    setEmpArrrr(filterEmpArr);
  }
  function removeEmployee(item, uuid) {
    setEmpArrrr([...empArrrr, item]);
    let filterTeamArr = teamArr.filter((item) => {
      return item.id != uuid;
    });
    setTeamArr(filterTeamArr);
  }

  return (
    <main className="main-box max-[768px]:py-6 min-[768px]:h-[100vh] w-full border-2 border-black flex max-[768px]:flex-col justify-center gap-4 items-center relative bg-[url('https://images.unsplash.com/photo-1541746972996-4e0b0f43e02a?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] bg-no-repeat bg-cover bg-center text-white">
      <Employee empArrrr={empArrrr} addEmployee={addEmployee} />
      <Team teamArr={teamArr} removeEmployee={removeEmployee} age={age} />
    </main>
  );
};
